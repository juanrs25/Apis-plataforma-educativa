  ...
  <div class="login-page">
    <div class="login-container">
      ...
      <form @submit.prevent="loginUsuario">
        ...
        <input id="username" v-model="form.Usuario" placeholder="Enter your username" required />

        <label for="password">Password</label>
        <input id="password" v-model="form.Clave" type="password" placeholder="Enter your password" required />
...
const loginUsuario = async () => {
  try {
    ...
    auth.login(res.data.token, res.data.rol)
    mensaje.value = res.data.message
    router.push('/dashboard')
  } catch (err) {
    mensaje.value = err.response?.data?.message || 'Error en el inicio de sesión'
  }
}
</script>
...
.login-container {
    ...
    box-shadow: 
        0 4px 15px rgba(0, 0, 0, 0.5), /* Sombra de profundidad */
        0 0 20px rgba(4, 24, 248, 0.4); /* Resplandor verde tenue */
